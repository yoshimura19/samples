data5 <- read.csv("/Users/serubantesu2/Downloads/yoshimura_kadai.csv",header= T,sep=",",na.strings=".",fileEncoding="cp932") 
attach(data5)
ls(data5)
...
var3 <- function(n)
{
  ...
  for(i in 1:n){
    ...
    d11 <- M[1,1]                          　#4×4用でadhocだが手計算で導出した
    ...
    predicted_y_J[i] <-
      - (B0[2,1]*predicted_y_C[i]) + betaJ[1] +          #中国外生の影響を今期も受ける
...
vartrade <- function(ep)    #short sell version(空売りver)    (x,istart,n)
{
