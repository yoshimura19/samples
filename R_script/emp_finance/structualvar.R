...
#チェック用
JPN1 <- nikkei225_1[(past +1):start]      #startのデータは今期-前期のnikkei
GER1 <- dax_1[(past+1):start]
USA1 <- S.P500_1[(past+1):start]
CHI1 <- SSE_composite_index_1[(past+1):start]

library(vars)
VARselect(m.ibmspln, lag.max = 5, type="const")  #SC(BIC)
try_x <- cbind(CHI1,JPN1,GER1,USA1)
var0 <- VAR(try_x, p=2, type = "const")
options(scipen=10)
summary(var0)
