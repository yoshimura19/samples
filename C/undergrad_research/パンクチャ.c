...
int n;                                               // 符号長(符号化後(検査行列))
int m;                                               // 検査記号数
int iteration=15;                                    // 反復回数
int trial=1;                                         // 試行回数
double P0 = 0.4;                                    // パンクチャ比率
double R = 0.5;                                      // 符号化率
double post_R; //= R / (1 - P0);                        // パンクチャ後符号化率
int count_punctured_bit;                             // パンクチャ数
double sigma, midium=0,SN=4;                         // 雑音のパラメタ (SN比で計算)
...
int *recoverable;                        // 復元場所(列番号)

/************ メイン関数 ***************************************************************/
int main(void)
{
     FILE *fp;
     int i=0,j=0,k=0,z=0,h=0,loop=0;                          // ループカウンタ
...
void check_matrix(FILE *fp){
     ...
     recoverable = (int *)malloc(sizeof(int) * n );      //n*P0+10
}
/*** 出力 **** 検査行列読み込み確認用 ***********************************************/
